...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      /*
       * Explicitly check pointerAngle again here so TypeScript knows
       * that target can never be null.
       */
      const target =
      ...
      const diff =
        ((target - angle + Math.PI * 3) % (Math.PI * 2)) -
        Math.PI;
      ...
      program.uniforms.uRadius.value =
        Math.min(
          p.radius,
          Math.min(sizeRef.w, sizeRef.h) / 2
        ) * dpr;
  ...
  return (
    ...
    >
      ...
      <span className="relative z-[2] font-sans font-medium">
        {children}
      </span>
    </button>
  );
